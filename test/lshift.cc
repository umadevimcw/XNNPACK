...
using LShiftTestS16 = BinaryTest<int16_t>;
using LShiftTestS32 = BinaryTest<int32_t>;


// Cap operations applied to logits (Q * K) of attention operator.
TEST_F(LShiftTestS16, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
                          subgraph, xnn_datatype_int16, dims.size(), dims.data(), nullptr,
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
                          subgraph, xnn_datatype_int16, dims.size(), dims.data(), nullptr, XNN_INVALID_VALUE_ID,
                          /*flags=*/0, &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);

  ASSERT_EQ(
    xnn_status_success,
    xnn_define_lshift(subgraph, input1_id, input2_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_lshift);
...
TEST_F(LShiftTestS16, matches_operator_api)
{
  std::generate(input1.begin(), input1.end(), [&]() { return s16dist(rng); });
  std::generate(input2.begin(), input2.end(), [&]() { return s16dist(rng); });
  std::fill(operator_output.begin(), operator_output.end(), INT16_MAX);
  std::fill(subgraph_output.begin(), subgraph_output.end(), INT16_MAX);
  ...
  // Call operator API.
  ASSERT_EQ(
    xnn_status_success, xnn_create_lshift_nd_s16(
                          /*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_lshift_nd_s16(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success, xnn_setup_lshift_nd_s16(op, input1.data(), input2.data(), operator_output.data()));
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
                          subgraph, xnn_datatype_int16, input1_dims.size(), input1_dims.data(), nullptr,
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_tensor_value(
                          subgraph, xnn_datatype_int16, input2_dims.size(), input2_dims.data(), nullptr,
  ...
  ASSERT_EQ(
    ...
    xnn_define_tensor_value(
      subgraph, xnn_datatype_int16, output_dims.size(), output_dims.data(), nullptr, /*external_id=*/2,
...
TEST_F(LShiftTestS32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(
    xnn_status_success, xnn_create_lshift_nd_s32(
                          /*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_lshift_nd_s32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success, xnn_setup_lshift_nd_s32(op, input1.data(), input2.data(), operator_output.data()));
