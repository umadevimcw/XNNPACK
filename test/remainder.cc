...
using RemainderTestQS16 = BinaryTest<int16_t>;
...
TEST_F(RemainderTestQS16, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success,
    xnn_define_remainder(subgraph, output_min, output_max, input1_id, input2_id, output_id, /*flags=*/0));
...
TEST_F(RemainderTestQS16, matches_operator_api)
{
  const int32_t input1_zero_point = s16dist(rng);
  const float input1_scale = scale_dist(rng);
  const int32_t input2_zero_point = s16dist(rng);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_reshape_remainder_nd_qs16(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_remainder_nd_qs16(op, input1.data(), input2.data(), operator_output.data()));
