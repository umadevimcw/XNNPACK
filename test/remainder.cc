...
using RemainderTestS32 = BinaryTest<int32_t>;
...
TEST_F(RemainderTestS32, define) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(RemainderTestS32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_remainder_nd_s32(/*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_remainder_nd_s32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_remainder_nd_s32(op, input1.data(), input2.data(), operator_output.data()));
