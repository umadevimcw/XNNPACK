...
class ReduceWindowMicrokernelTester {
 public:
  ...
  void Test(xnn_f32_reducewindow_ukernel_fn reducewindow, OpType op_type, xnn_init_f32_reducewindow_params_fn init_params) const {
    ...
    std::uniform_int_distribution<int32_t> s32dist(0, 10);
    ...
    for (int32_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      printf("##################################\n");
      printf("pad_row = %d, pad_col = %d, out_row = %d, out_col = %d\n",pad_row, pad_col, out_row, out_col );
      ...
      for (int32_t i = 0; i < out_row * out_col; i++) {
        ...
        //EXPECT_NEAR(output[i], output_ref[i], std::abs(output_ref[i]) * 1.0e-6f)
