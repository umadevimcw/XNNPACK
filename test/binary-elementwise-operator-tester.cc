  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    switch (operation_type()) {
      case OperationType::LShift:
        ASSERT_EQ(xnn_status_success,
                  xnn_create_lshift_nd_s16(0, &binary_elementwise_op));
        break;
      default:
        FAIL() << "Unsupported operation type";
    }
    ...
    switch (operation_type()) {
      case OperationType::LShift:
        ASSERT_EQ(
            xnn_status_success,
            xnn_reshape_lshift_nd_s16(
                binary_elementwise_op, num_input1_dims(), input1_shape().data(),
                num_input2_dims(), input2_shape().data(),
                /*threadpool=*/nullptr));
        ASSERT_EQ(xnn_status_success, xnn_setup_lshift_nd_s16(
                                          binary_elementwise_op, input1.data(),
                                          input2.data(), output.data()));
        break;
      default:
...
void BinaryElementwiseOperatorTester::TestS32() const {
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    switch (operation_type()) {
      case OperationType::LShift:
        ASSERT_EQ(xnn_status_success,
                  xnn_create_lshift_nd_s32(0, &binary_elementwise_op));
        break;
      case OperationType::Multiply:
        ASSERT_EQ(xnn_status_success,
    ...
    switch (operation_type()) {
      case OperationType::LShift:
        ASSERT_EQ(
            xnn_status_success,
            xnn_reshape_lshift_nd_s32(
                binary_elementwise_op, num_input1_dims(), input1_shape().data(),
                num_input2_dims(), input2_shape().data(),
                /*threadpool=*/nullptr));
        ASSERT_EQ(xnn_status_success, xnn_setup_lshift_nd_s32(
                                          binary_elementwise_op, input1.data(),
                                          input2.data(), output.data()));
        break;
      case OperationType::Multiply:
