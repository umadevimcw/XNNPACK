...
void VBinaryCMicrokernelTester::Test(
    xnn_qs16_vmin_ukernel_fn vbinary, OpType op_type,
    xnn_init_qs16_min_minmax_params_fn init_params,xnn_qs16_requantize_fn requantize) const {
  xnnpack::ReplicableRandomDevice rng;
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    const float output_scale = scale / y_scale();
    xnn_qs16_min_minmax_params params;
    if (init_params != nullptr) {
      init_params(&params, a_zero_point_s16(), b_zero_point_s16(), output_scale, y_zero_point_s16(),qmin_s16(),qmax_s16());
    }

    // Compute reference results.
    for (size_t i = 0; i < batch_size(); i++) {
          const int32_t a_quantize = (static_cast<int32_t>(a_data[i]) - static_cast<int32_t>(a_zero_point_s16()));
          const int32_t b_quantize = (static_cast<int32_t>(b) - static_cast<int32_t>(b_zero_point_s16()));       
          const int32_t acc = (a_quantize < b_quantize) ? a_quantize : b_quantize;
          int32_t res = static_cast<int32_t>(y_zero_point_s16()) +
            static_cast<int32_t>(output_scale * static_cast<float>(acc));
          ...
          y_ref[i] = requantize(acc, output_scale, y_zero_point_s16(),
                                qmin_s16(), qmax_s16());
    }
    // Call optimized micro-kernel.
