...
using Minimum2TestQS16 = BinaryTest<int16_t>;
...
TEST_F(Minimum2TestQS16, define) {
...
TEST_F(Minimum2TestQS16, matches_operator_api)
{
  const int32_t input1_zero_point = s16dist(rng);
  const float input1_scale = scale_dist(rng);
  const int32_t input2_zero_point = s16dist(rng);
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_minimum_nd_qs16(input1_zero_point, input1_scale, input2_zero_point,
                                                           input2_scale, output_zero_point, output_scale, 
                                                           quantized_output_min, quantized_output_max, /*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_minimum_nd_qs16(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_minimum_nd_qs16(op, input1.data(), input2.data(), operator_output.data()));
