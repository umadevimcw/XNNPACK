...
{
  const struct xnn_binary_elementwise_config* s16_lshift_config = xnn_init_s16_vlshift_config();
  if (s16_lshift_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_lshift_nd_s16));
    return xnn_status_unsupported_hardware;
  }
  ...
  return create_binary_elementwise_nd(
    ...
    &params,
    sizeof(params),
    xnn_operator_type_lshift_nd_s16,
    &s16_lshift_config->linear,
    lshift_op_out);
}
...
enum xnn_status xnn_create_lshift_nd_s32(
    ...
    xnn_operator_t* lshift_op_out)
{
  const struct xnn_binary_elementwise_config* s32_lshift_config = xnn_init_s32_vlshift_config();
  if (s32_lshift_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_lshift_nd_s32));
    return xnn_status_unsupported_hardware;
  }
    ...
    pthreadpool_t threadpool)
{
  ...
  return reshape_binary_elementwise_nd(
    lshift_op, xnn_operator_type_lshift_nd_s16,
    num_input1_dims, input1_shape,
    num_input2_dims, input2_shape,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_INT16_T,
    &lshift_op->params.s16_default, sizeof(lshift_op->params.s16_default),
    &lshift_op->params.s16_default, sizeof(lshift_op->params.s16_default),
    threadpool);
}
    ...
    pthreadpool_t threadpool)
{
  ...
  return reshape_binary_elementwise_nd(
    lshift_op, xnn_operator_type_lshift_nd_s32,
    num_input1_dims, input1_shape,
    num_input2_dims, input2_shape,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_INT32_T,
    &lshift_op->params.s32_default, sizeof(lshift_op->params.s32_default),
    &lshift_op->params.s32_default, sizeof(lshift_op->params.s32_default),
    threadpool);
}
