...
enum xnn_status xnn_create_remainder_nd_s32(
    uint32_t flags,
    xnn_operator_t* remainder_op_out)
{
  const struct xnn_binary_elementwise_config* s32_remainder_config = xnn_init_s32_vrem_config();
  if (s32_remainder_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_remainder_nd_s32));
    return xnn_status_unsupported_hardware;
  }
  ...
  return create_binary_elementwise_nd(
    ...
    &params,
    sizeof(params),
    xnn_operator_type_remainder_nd_s32,
    &s32_remainder_config->linear,
    remainder_op_out);
}
...
enum xnn_status xnn_reshape_remainder_nd_s32(
    ...
    pthreadpool_t threadpool)
{
  ...
  return reshape_binary_elementwise_nd(
    rem_op, xnn_operator_type_remainder_nd_s32,
    num_input1_dims, input1_shape,
    num_input2_dims, input2_shape,
    /*log2_element_size=*/XNN_LOG2_SIZEOF_INT32_T,
    &rem_op->params.s32_default, sizeof(rem_op->params.s32_default),
    &rem_op->params.s32_default, sizeof(rem_op->params.s32_default),
    threadpool);
}
...
enum xnn_status xnn_setup_remainder_nd_s32(
    xnn_operator_t rem_op,
    const int32_t* input1,
    const int32_t* input2,
    int32_t* output)
