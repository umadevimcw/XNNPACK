...
enum xnn_status xnn_create_maximum_nd_qs16(
    ...
    xnn_operator_t* maximum_op_out)
{
   if (input1_scale <= 0.0f || !isnormal(input1_scale)) {
    xnn_log_error(
      "failed to create %s operator with %.7g input 1 scale: scale must be finite and positive",
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16), input1_scale);
    return xnn_status_invalid_parameter;
  }
  ...
  if (input2_scale <= 0.0f || !isnormal(input2_scale)) {
    xnn_log_error(
      "failed to create %s operator with %.7g input 2 scale: scale must be finite and positive",
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16), input2_scale);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_scale <= 0.0f || !isnormal(output_scale)) {
    xnn_log_error(
      "failed to create %s operator with %.7g output scale: scale must be finite and positive",
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16), output_scale);
    return xnn_status_invalid_parameter;
  }
  ...
  if (product_output_scale < 0x1.0p-16f || product_output_scale >= 0x1.0p+8f) {
    xnn_log_error(
      ...
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16), product_output_scale);
    return xnn_status_unsupported_parameter;
  }
  ...
  if (output_min > output_max) {
    xnn_log_error(
      ...
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16), output_min, output_max);
    return xnn_status_invalid_parameter;
  }

  const struct xnn_binary_elementwise_config* qs16_vmax_config =
      xnn_init_qs16_vmax_config();
  if (qs16_vmax_config == NULL) {
    xnn_log_error("failed to create %s operator: unsupported hardware configuration",
      xnn_operator_type_to_string(xnn_operator_type_maximum_nd_qs16));
    return xnn_status_unsupported_hardware;
  }

  union xnn_qs16_minmax_params params;
  union xnn_qs16_minmax_params params2;
  assert(qs16_vmax_config->init.qs16_minmax != NULL);
  qs16_vmax_config->init.qs16_minmax(&params, input1_zero_point, input2_zero_point,
                                  product_output_scale, output_zero_point,output_min,output_max);
  qs16_vmax_config->init.qs16_minmax(&params2, input2_zero_point,
  ...
  return create_binary_elementwise_nd(flags, &params, &params2, sizeof(params),
                                      xnn_operator_type_maximum_nd_qs16,
                                      &qs16_vmax_config->minmax,
                                      maximum_op_out);
}
...
enum xnn_status xnn_reshape_maximum_nd_qs16(
...
{
  return reshape_binary_elementwise_nd(
      max_op, xnn_operator_type_maximum_nd_qs16, num_input1_dims, input1_shape,
      num_input2_dims, input2_shape,
      /*log2_element_size=*/XNN_LOG2_SIZEOF_INT16_T, &max_op->params.qs16_minmax,
      sizeof(max_op->params.qs16_minmax), &max_op->params.qs16_minmax,
      sizeof(max_op->params.qs16_minmax), threadpool);
}
...
enum xnn_status xnn_setup_maximum_nd_qs16(
    xnn_operator_t max_op,
...
{
  return setup_binary_elementwise_nd(
    max_op, xnn_operator_type_maximum_nd_qs16,
    input1, input2, output);
}
