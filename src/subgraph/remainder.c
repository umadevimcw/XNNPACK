...
static enum xnn_status create_remainder_operator(
  ...
  xnn_weights_cache_t weights_cache)
{
  ...
  switch (node->compute_type) {
    case xnn_compute_type_fp32:
      status = xnn_create_remainder_nd_f32(
        node->flags,
...
static enum xnn_status reshape_remainder_operator(
  struct xnn_operator_data* opdata,
  ...
  pthreadpool_t threadpool)
{
  ...
  switch (opdata->operator_objects[0]->type) {
    case xnn_operator_type_remainder_nd_f32:
      status = xnn_reshape_remainder_nd_f32(
        opdata->operator_objects[0],
...
static enum xnn_status setup_remainder_operator(
...
{
  enum xnn_status status;
  if ((status = xnn_subgraph_check_xnnpack_initialized(xnn_node_type_remainder)) != xnn_status_success) {
    return status;
  }

  if ((status = xnn_subgraph_check_nth_input_node_id(xnn_node_type_remainder, input1_id, subgraph->num_values, 1)) !=
      xnn_status_success) {
    return status;
  }

  const struct xnn_value* input1_value = &subgraph->values[input1_id];
  status = xnn_subgraph_check_nth_input_type_dense(xnn_node_type_remainder, input1_id, input1_value, 1);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (input1_value->datatype) {
    ...
    default:
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_remainder), input1_id,
        xnn_datatype_to_string(input1_value->datatype), input1_value->datatype);
      return xnn_status_invalid_parameter;
  }

  if ((status = xnn_subgraph_check_nth_input_node_id(
        xnn_node_type_remainder, input2_id, subgraph->num_values, 2)) != xnn_status_success) {
    return status;
  }

  const struct xnn_value* input2_value = &subgraph->values[input2_id];
  status = xnn_subgraph_check_nth_input_type_dense(xnn_node_type_remainder, input2_id, input2_value, 2);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (input2_value->datatype) {
    ...
    default:
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_remainder), input2_id,
        xnn_datatype_to_string(input2_value->datatype), input2_value->datatype);
      return xnn_status_invalid_parameter;
  }

  status = xnn_subgraph_check_output_node_id(xnn_node_type_remainder, output_id, subgraph->num_values);
  if (status != xnn_status_success) {
    return status;
  }

  const struct xnn_value* output_value = &subgraph->values[output_id];
  status = xnn_subgraph_check_output_type_dense(xnn_node_type_remainder, output_id, output_value);
  if (status != xnn_status_success) {
    return status;
  }
  ...
  switch (output_value->datatype) {
    ...
    default:
      xnn_log_error(
        ...
        xnn_node_type_to_string(xnn_node_type_remainder), output_id,
  ...
  node->create = create_remainder_operator;
  node->reshape = reshape_remainder_operator;
  node->setup = setup_remainder_operator;
