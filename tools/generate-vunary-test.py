...
def split_ukernel_name(name):
  match = re.fullmatch(
      r"(?:xnn_|xnn_generate_)(s8|u8|bf16|f16|f32|u32|u64|s32|s16)(_(s8|u8|bf16|f16|f32|u32|u64))*_v(abs|clamp|elu|exp|gelu|hswish|log|lrelu|neg|relu|rndd|rndne|rndu|rndz|rsqrt|sigmoid|sqr|sqrt|sqrtshift|tanh|clz)_(fact_)?ukernel__(.+)_u(\d+)(v)?",
      name,
  )
  if match is None:
